...
describe('Pruebas del componente Ticket Detail View', () => {
  ...
  it('Debe mostrar la información Prioridad', () => {
    ...
    cy.get('.flex-container').contains('Urgente');
  });

  it('Debe mostrar la información Categoría', () => {
    cy.get('.titleTicket').should('exist');  // Verifica que el elemento exista
    cy.get('.titleTicket').contains('Asistencia');
  });
